...
class DNIN_set(object):
    ...
    def __add__(self, otherList):
        ...
        for i in self.container:
            myList.append(i)
        if isinstance(otherList, MyCustomDNNeuronClass.DNinputNeuron):
    ...
    def LC4_LO_Filter(self):
        ...
        for x in self:
            LOtnid = []
            LOtnid = GN.getDownStreamNodes(skeletonID=x.skeletonID, tag='startOfLobula')
    ...
    def importClusters(self, selection = None):
        if selection is not None:
            df=pandas.read_csv('C:/Users/polskyj/myCode/RCode/newTest/{}.csv'.format(selection),converters={"connectorID":int, 'X':float,'Y':float, 'Z':float, 'Cluster':int})
        else:    
    ...
    def importSubClusters(self, fileName = None):
        ...
        df = pandas.read_csv(fileAddress, converters={"connectorID":int, 'X':float,'Y':float, 'Z':float, 'Cluster':int, 'subCluster':int})
        for index, row in df.iterrows():
            if 'tempHolder' in self[int(row['skeletonID'])].subClusters:
                self[int(row['skeletonID'])].subClusters[str(row['Cluster'])] = self[int(row['skeletonID'])].subClusters['tempHolder']
    ...
    def makeQuantiles(self):
        ...
        while (c <= 3):
            ...
            qs[c]['percent of all {} synapses on DNright'.format(self.groupName)] = (tempVar.AllDNrightSynapses/mySetSynCount)
    ...
    def getConnectorsInfo(self):
        mySkids = []
        for i in self:
            mySkids.append(i.skeletonID)
        myConnectorInfo = GC.getConnectorPartners(GC.getConnectors(mySkids, 'pre'))
...
def buildFromSkidList(myList):    
    mySet = DNIN_set(np.array(MyCustomDNNeuronClass.buildFromSkidList(myList)))    
    return mySet

#sorts neurons from low to high synapse counts
def sortBySynL2H(mySet):
    L2H = sorted(mySet, key = MyCustomDNNeuronClass.DNinputNeuron.getDNrightsynapse)
    mySortedList = subSetBuilder(L2H)
    if mySet.groupName != None:
        mySortedList.groupName = mySet.groupName
    if mySet.myDN is not None:
        mySortedList.myDN = mySet.myDN
    if mySet.myDNskid is not None:
        mySortedList.myDNskid = mySet.myDNskid
    return mySortedList

#sorts neurons from low to high synapse counts
def sortBySynH2L(mySet):
    H2L = sorted(mySet, key = MyCustomDNNeuronClass.DNinputNeuron.getDNrightsynapse, reverse=True)
...
def subSetBuilder(fullSet):
    ...
    if isinstance(fullSet, DNIN_set):
        ...
        if fullSet.myDN is not None:
            mySubSet.myDN = fullSet.myDN
        if fullSet.myDNskid is not None:
            mySubSet.myDN = fullSet.myDNskid
...
def makeDistributedGroupsByColor(mySet):
    ...
    while (c <= 3):
        ...
        myBigSet[c] = tempVar
        myTempSet -= tempVar
        myAdditionalInfo += "\n quantile with <= {} synapses onto DN:  \n      - {} neurons in this quantile \n     -average number of synapses: {} \n     -median number of synapses: {} \n      - total number of synapses onto DN in this quantile: {} \n     - % of all {} synapses onto DN: {} \n".format(a[c], tempVar.numNeurons, tempVar.avg(), tempVar.med(), tempVar.AllDNrightSynapses, mySet.groupName, (tempVar.AllDNrightSynapses/mySetSynCount))
        tempVar = None
        c += 1  
    tempVar = subBySynGT(myTempSet, a[c-1])
    myBigSet[c] = tempVar
    myAdditionalInfo += "\n quantile with > {} synapses onto DN:  \n      - {} neurons in this quantile \n     -average number of synapses: {} \n     -median number of synapses: {} \n      - total number of synapses onto DN in this quantile: {} \n     - % of all {} synapses onto DN: {}".format(a[c-1], tempVar.numNeurons, tempVar.avg(), tempVar.med(), tempVar.AllDNrightSynapses, mySet.groupName, (tempVar.AllDNrightSynapses/mySetSynCount))    
    for i in myBigSet:
...
def appendNonDNIN(mySet, annotation):
    ...
    for item in testList:
        if item not in curSKIDs:
            myNeuron = MyCustomDNNeuronClass.buildSingleCell(int(item))
            mySet += myNeuron
    return mySet
def appendNonDNIN2(mySet, annotation):
    ...
    for item in testList:
        if item not in curSKIDs:
            myNeuron = MyCustomDNNeuronClass.buildSingleCellQuick(int(item))
            mySet += myNeuron
    return mySet    
