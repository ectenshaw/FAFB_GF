...
# Written by Markus Pleijzier & Philipp Schlegel
...
http_user = "polskyj"
http_pw = config.http_password
token = config.token

pymaid.CatmaidInstance(server, http_user, http_pw, token)

# This is an integer, unique to your neuron(s) of interest

Neuron_1_skeleton_id_number = 4947529
Neuron_1 = pymaid.get_neuron(Neuron_1_skeleton_id_number)
...
# This is the plotting function, with a description of the arguments & an example.
...
def plot_nx(x, plot_connectors=True, highlight_connectors=None, prog='dot'):
    ...
    if highlight_connectors != None:
        hl_cn_coords = np.array(
            [pos[tn] for tn in x.connectors[x.connectors.connector_id.isin(highlight_connectors)].treenode_id])
        plt.scatter(hl_cn_coords[:, 0], hl_cn_coords[:, 1], s=1, c=(0.8, 0.0, 0.2), zorder=3)

    print('Done in %is' % int(time.time() - start))


# Once the function has been ran, then it needs to be called...
...
# To change the width of the line, change the width argument in the nx.draw() argument.

# To change the colours of the presynapses (relation == 0), postsynapses (relation == 1) and connectors of interest,
# change the RGB values in the c=(.0,.0,.0) arguments of plt.scatter. Manipulating the s argument within plt.scatter()
# changes the size of the circles that are representing the connectors.

# Saving the file as svg maintains the best quality, however the image can also be saved as png, jpeg etc.
# Once the svg file is plotted, it can be aesthetically manipulated in Adobe Illustrator or some other design platform.
