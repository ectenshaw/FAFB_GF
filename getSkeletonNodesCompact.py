...
'''#input is a single object of type myCustomGFNeuron, output is a list of integers representing the x,y, and z coordinate of the soma in catmaid. BUG: Cannot find somata of all LPLC2 - fix this
def getSoma(myNeur):
    skeletonID = myNeur.skeletonID
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/1/skeletons/{}/node-overview'.format(skeletonID), 
...
#input is a single object of type myCustomGFNeuron, output is a list of integers representing the x,y, and z coordinate of the soma in catmaid. if desired soma node number can be returned by changing line 200 to ... = [x, z]
...
def getAllNodes4BranchFilter():
    ...
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/{}/skeletons/{}/compact-detail?with_connectors=true'.format(project_id, mySkid), 
            auth = auth
    )
...
#returns list [myNodesAndSuch, connectors] --> connectors = [tnid, connectorID, synaptic polarity(0=pre, 1=post), x, y, z]
def getAllNodesDownStream(TNID, notTNID = None, skelInfo = None):
    if skelInfo is None:    
        skelInfo = getAllNodes4BranchFilter()
    else:
        skelInfo = skelInfo
    allNodes = skelInfo[0]
    myPID = TNID[0]
    ...
    while not len(candidates) == 0:
        ...
        candidates = candidates + children
        myListAndConnectors = []
        myListAndConnectors.append(myList)
        myListAndConnectors.append(skelInfo[1])
    return myListAndConnectors
...
def getDownStreamNodes(skeletonID, tag, myCurNodes = None, skelInfo = None):
    if skeletonID is None:
        skeletonID = 4947529
    TNID = getTnid(tag, skeletonID)
    if myCurNodes is None:
        a = getAllNodesDownStream(TNID, skelInfo = skelInfo)
        a = a[0]
    else:
        a = myCurNodes
...
'''def getAllConnectorInfo(skeletonID = None, polarity = None):
    ...
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/{}/connectors/links/?skeleton_ids[0]={}&relation_type={}_to&with_tags=true'.format(project_id, mySkid, relation_type)#, 
            #auth = auth,
...
#myCompactList[1] should be in form: [TNID, connectorID, polarity, x, y, z]
def getConnectorInfoCompact(tag, myCompactList, skeletonID = None, myCurNodes = None, polarity = None, connectorInfo = None):
    if skeletonID is None:
        skeletonID = 4947529
    connectorInfo = myCompactList[1]
    if myCurNodes is None:
        downStreamNodes = getDownStreamNodes(skeletonID, tag, skelInfo = myCompactList)
    else:
        downStreamNodes = myCurNodes
    myConnectorIDs = []
    for item in connectorInfo:
        if item[0] in downStreamNodes:
            myConnectorIDs.append(item[1])
    return myConnectorIDs
...
def getConnectorsByBranch(branch, skeletonID = None, connectorInfo = None, myCompactList = None):
    myCompactInfo = getAllNodes4BranchFilter()
    branch = str(branch)
    if branch == 'anterior' or branch == 'medial':
        ...
        myConnectors = getConnectorInfoCompact(tag = 'start of {} branch'.format(branch), myCompactList = myCompactInfo, connectorInfo = connectorInfo)
        ...
        #theseNodes = getAllNodesDownStreamCompact(myTNID, notTNID, skelInfo = )
        theseNodes = getAllNodesDownStream(myTNID, notTNID, skelInfo = myCompactInfo)
        myCurNodes = getDownStreamNodes(skeletonID, tag = 'start of lateral branch', myCurNodes = theseNodes, skelInfo = myCompactInfo)
        myConnectors = getConnectorInfoCompact('start of {} branch'.format(branch), skeletonID = None, myCompactList = myCompactInfo, myCurNodes = myCurNodes)
        ...
        myTNID = getTnid('soma')
        theseNodes = getAllNodesDownStream(myTNID, notTNID, skelInfo = myCompactInfo)
        myCurNodes = getDownStreamNodes(skeletonID, tag = 'soma', myCurNodes = theseNodes, skelInfo = myCompactInfo)
        myConnectors = getConnectorInfoCompact('soma'.format(branch), skeletonID = None, myCompactList = myCompactInfo, myCurNodes = myCurNodes)        
    return myConnectors
