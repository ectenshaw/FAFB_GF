...
def getSomaNew(myNeuron):
    skeletonID = myNeur.skeletonID
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/1/skeletons/2161608/node-overview'.format(skeletonID),
            #data = {'skeleton_ids' : myNeurons[0].skeletonID}
    )
    myDict = json.loads(response.content)
    somaAndCoordinates = [None] * 3
    a = None
...
def getSkidSomaDict(neuronSet):
    skeletonID = neuronSet.skeletonID
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/1/skeletons/{}/node-overview'.format(skeletonID),
    )
    myDict = json.loads(response.content)
