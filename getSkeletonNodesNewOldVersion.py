...
'''def getAllNodesDownStream(TNID):
    ...
    skelInfo = json.loads(response.content)
    skelInfo = skelInfo[0:2]   
    for item in skelInfo[0]:
        if item[0] < TNID:
            skelInfo[0].remove(item)
...
#returns list of all connector nodes(postsynaptic) within the range of downstream tag
def getConnectorInfo(tag, skeletonID = None, myCurNodes = None):
    ...
    response = requests.get(
            'https://neuropil.janelia.org/tracing/fafb/v14/{}/connectors/links/?skeleton_ids[0]=4947529&relation_type=postsynaptic_to&with_tags=true'.format(project_id)#, 
            #auth = auth,
...
def getNumSynByBranch(branch, skeletonID = None):
    ...
    if branch == 'anterior' or branch == 'medial':
        myConnectors = getConnectorInfo('start of {} branch'.format(branch))
    elif branch == 'lateral':
        notTNID = []
